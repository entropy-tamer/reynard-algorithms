...
**Overlap Dimensions**:
$$\text{overlap}_x = \max(0, \min(x_1 + w_1, x_2 + w_2) - \max(x_1, x_2))$$
...
Based on comprehensive benchmarking (see `src/__tests__/readme-benchmarks.test.ts`):
...
*Benchmark methodology: Each test runs multiple iterations on Intel i5-1135G7 @ 2.40GHz with dataset characteristics documented in test file.*
...
Based on comprehensive stress testing (see `src/__tests__/readme-benchmarks.test.ts`):
...
*See `src/__tests__/readme-benchmarks.test.ts` for full stress test implementation and methodology*
