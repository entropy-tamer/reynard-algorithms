...
export function createVoxelGrid(
  ...
  const voxels: Voxel[][][] = Array.from({ length: gridZ }, () =>
    Array.from({ length: gridY }, () => Array.from({ length: gridX }, () => ({
 ...
 */
function queryVoxelPointsRecursive(
  node: OctreeNode,
  voxelBounds: Bounds3D,
  points: Point3D[]
): void {
...
export function getVoxelAt(
  ...
  if (
    x < 0 || x >= voxelGrid.dimensions.x ||
    y < 0 || y >= voxelGrid.dimensions.y ||
    z < 0 || z >= voxelGrid.dimensions.z
  ) {
    return null;
  }
    ...
    for (let y = 0; y < voxelGrid.dimensions.y; y++) {
      for (let x = 0; x < voxelGrid.dimensions.x; x++) {
        ...
        const vBounds: Bounds3D = {
          ...
          max: { x: v.position.x + voxelGrid.voxelSize, y: v.position.y + voxelGrid.voxelSize, z: v.position.z + voxelGrid.voxelSize },
          center: { x: v.position.x + voxelGrid.voxelSize / 2, y: v.position.y + voxelGrid.voxelSize / 2, z: v.position.z + voxelGrid.voxelSize / 2 },
  ...
  for (let z = 0; z < voxelGrid.dimensions.z; z++) {
    ...
    for (let y = 0; y < voxelGrid.dimensions.y; y++) {
      for (let x = 0; x < voxelGrid.dimensions.x; x++) {
        ...
        if (!v) { slice.push(0); continue; }
        if (!root) { slice.push(v.occupied ? 1 : 0); continue; }
        const vb: Bounds3D = {
          min: { x: v.position.x, y: v.position.y, z: v.position.z },
          max: { x: v.position.x + voxelGrid.voxelSize, y: v.position.y + voxelGrid.voxelSize, z: v.position.z + voxelGrid.voxelSize },
          center: { x: v.position.x + voxelGrid.voxelSize / 2, y: v.position.y + voxelGrid.voxelSize / 2, z: v.position.z + voxelGrid.voxelSize / 2 },
