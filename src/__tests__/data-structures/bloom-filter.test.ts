...
import { BloomFilterEventType } from "../../data-structures/bloom-filter/bloom-filter-types";
...
describe("Bloom Filter Data Structure", () => {
  ...
  describe("Performance Benchmarks", () => {
    ...
    runBenchmark("large dataset insertion", 1000, (filter, elements) => {
      elements.forEach(element => filter.insert(element));
    });

    runBenchmark("large dataset testing", 1000, (filter, elements) => {
      // First insert some elements
      elements.slice(0, 500).forEach(element => filter.insert(element));
      // Then test all elements
      elements.forEach(element => filter.test(element));
    });
  });
