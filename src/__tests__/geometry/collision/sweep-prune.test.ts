...
describe("Sweep and Prune Collision Detection", () => {
  ...
  describe("Mathematical Theory", () => {
    ...
    it("should respect the fundamental sweep and prune principle: sort and sweep", () => {
      ...
      const hasCollision = result.collisionPairs.some(
        pair =>
  ...
  describe("Caching", () => {
    ...
    it("should clear cache when requested", () => {
      ...
      // Second call should use cache (executionTime = 0)
      const result2 = sweepPrune.detectCollisions(aabbs);
      ...
      // Next call should not use cache (execution time should be calculated)
      const result3 = sweepPrune.detectCollisions(aabbs);
      expect(result3.collisionPairs.length).toBeGreaterThan(0);
      expect(result3.executionTime).toBeGreaterThanOrEqual(0); // Should be recalculated, not from cache
    });
  });
  ...
  describe("Integration", () => {
    it("should work with common AABB shapes", () => {
      const commonAABBs = [COMMON_AABBS.UNIT_SQUARE, COMMON_AABBS.OFFSET_SQUARE, COMMON_AABBS.LARGE_SQUARE];
