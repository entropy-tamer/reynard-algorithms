...
export function propagateConstraints(
  ...
  while (queue.length > 0) {
    ...
    for (const direction of directions) {
      ...
      // Apply constraints
      const wasModified = applyConstraintsToNeighbor(
        currentCell,
        neighborCell,
        constraints,
        direction
...
function applyConstraintsToNeighbor(
  ...
  for (const tile of currentCell.possibleTiles) {
    const tileConstraints = constraints.filter(c => 
      c.tile1 === tile && c.direction === direction
    );
 ...
 */
export function isGridFullyCollapsed(
  grid: Cell[][][],
  width: number,
  height: number,
  depth: number
