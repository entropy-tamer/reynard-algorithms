...
import { Point, LineSegment, SweepLineEvent, EventQueue, StatusStructure, StatusNode } from "./line-intersection-types";
...
export class SweepLineUtils {
   ...
   * @example
   */
  static createIntersectionEvent(point: Point, segment1: LineSegment, segment2: LineSegment): SweepLineEvent {
    return {
      type: "intersection",
      point,
   ...
   * @example
   */
  static pointOnSegment(point: Point, segment: LineSegment, tolerance: number = 1e-10): boolean {
    const { start, end } = segment;
