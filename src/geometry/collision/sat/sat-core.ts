...
import { DEFAULT_SAT_CONFIG, SATEventType } from "./sat-types";
...
export class SAT {
  ...
  private performSATTest(polygon1: ConvexPolygon, polygon2: ConvexPolygon): SATCollisionResult {
    ...
    for (const axis of axes) {
      ...
      if (
        overlap < minOverlap ||
        (Math.abs(overlap - minOverlap) <= this.config.epsilon &&
          separationAxis &&
  ...
  private findContactPoints(
    ...
    _separationAxis: ProjectionAxis
  ): Point2D[] {
  ...
  private isDegenerate(p: ConvexPolygon): boolean {
