...
describe("Memoization Performance Benchmarks", () => {
  ...
  describe("Mathematical Operations", () => {
    it(
      "should demonstrate significant performance improvement for repeated square calculations",
      { timeout: 3000 },
      () => {
  ...
  describe("Geometric Calculations", () => {
    it("should demonstrate performance improvement for distance calculations", { timeout: 4000 }, () => {
      ...
      const iterations = 1;
      const pairIndices: Array<[number, number]> = Array.from({ length: 20 }, (_, i) => [
        i,
        (i + 1) % testPoints.length,
      ]);
      const repeats = 50;
