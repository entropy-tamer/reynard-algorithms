...
export class FlowField {
  ...
  generateFlowField(
    ...
    try {
      ...
      this.integrationField = generateIntegrationField(
        grid,
        width,
        height,
        goals,
      ...
      const errorStats = { ...this.stats, success: false, error: error instanceof Error ? error.message : "Unknown error" };
      return {
        success: false,
        flowField: [],
        integrationField: [],
  ...
  private calculateStats(): void {
    const costs = this.integrationField
      .filter(cell => cell.cost < this.config.maxCost)
      .map(cell => cell.cost);
