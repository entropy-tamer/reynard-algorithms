...
export function compareFlowFields(
    ...
    if (cellComparison.identical) {
      ...
      if (comparisonOptions.detailedAnalysis) {
        if (cellComparison.magnitudeDifferent) {
          differences.push(`Magnitude difference at (${point.x}, ${point.y}): ${cell1.magnitude} vs ${cell2.magnitude}`);
          stats.magnitudeDifferences++;
        }
        
        if (cellComparison.directionDifferent) {
          differences.push(`Direction difference at (${point.x}, ${point.y}): (${cell1.x}, ${cell1.y}) vs (${cell2.x}, ${cell2.y})`);
  ...
  stats.averageMagnitudeDifference = stats.differentCells > 0 
    ? totalMagnitudeDifference / stats.differentCells 
    : 0;
...
export function compareIntegrationFields(
  ...
  stats.averageMagnitudeDifference = stats.differentCells > 0 
    ? totalCostDifference / stats.differentCells 
    : 0;
...
export function compareFlowFieldResults(
  ...
  if (comparisonOptions.compareIntegrationFields && result1.integrationField && result2.integrationField && result1.width && result1.height) {
...
export function calculateSimilarity(
  ...
  for (let i = 0; i < flowField1.length; i++) {
    ...
    const magnitudeSimilarity = 1 - Math.abs(cell1.magnitude - cell2.magnitude) / Math.max(cell1.magnitude, cell2.magnitude);
