 ...
 * @example
 */
function transformOctant(originX: number, originY: number, col: number, row: number, octant: number): [number, number] {
  switch (octant) {
    case 0:
      return [originX + col, originY - row];
    case 1:
      return [originX + row, originY - col];
    case 2:
      return [originX + row, originY + col];
    case 3:
      return [originX + col, originY + row];
