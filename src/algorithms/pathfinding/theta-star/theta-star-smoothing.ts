...
import type { Point, PathOptimizationOptions, PathOptimizationResult } from "./theta-star-types";
 ...
 * @example
 */
export function removeRedundantPoints(path: Point[], grid: CellType[], width: number, height: number): Point[] {
  if (path.length <= 2) {
    return path;
  }
...
export function smoothPath(
  ...
  for (let iteration = 0; iteration < maxIterations; iteration++) {
    ...
    for (let i = 1; i < smoothedPath.length - 1; i++) {
      ...
      if (isValidPoint(smoothedPoint, width, height) && isWalkable(grid, smoothedPoint, width, height)) {
 ...
 * @example
 */
function hasLineOfSight(from: Point, to: Point, grid: CellType[], width: number, height: number): boolean {
 ...
 * @example
 */
export function compressPath(path: Point[], grid: CellType[], width: number, height: number): Point[] {
  if (path.length <= 2) {
    return path;
  }
...
export function relaxPath(
  ...
  for (let i = 1; i < path.length - 1; i++) {
    ...
    if (isValidPoint(relaxedPoint, width, height) && isWalkable(grid, relaxedPoint, width, height)) {
