...
import type { Point, FlowCell, FlowFieldValidationOptions, FlowFieldValidationResult } from "./flow-field-types";
  ...
  for (let i = 0; i < flowField.length; i++) {
    ...
    if (validationOptions.checkDirection) {
      ...
      if (magnitudeDiff > (validationOptions.tolerance || 1e-10)) {
        warnings.push(
          `Magnitude mismatch at (${point.x}, ${point.y}): expected ${expectedMagnitude}, got ${cell.magnitude}`
        );
      }
    }
  }
 ...
 */
function checkFlowFieldConsistency(flowField: FlowCell[], width: number, height: number): string[] {
  const issues: string[] = [];
 ...
 */
function detectCircularFlows(flowField: FlowCell[], width: number, height: number): Point[] {
 ...
 */
function findCycle(start: Point, flowField: FlowCell[], width: number, height: number, visited: Set<string>): Point[] {
  const path: Point[] = [];
 ...
 */
function getNextPoint(point: Point, flowField: FlowCell[], width: number, height: number): Point | null {
 ...
 */
function detectIsolatedRegions(flowField: FlowCell[], width: number, height: number): Point[][] {
  const visited = new Set<string>();
  const regions: Point[][] = [];
 ...
 */
function floodFill(start: Point, flowField: FlowCell[], width: number, height: number, visited: Set<string>): Point[] {
  const region: Point[] = [];
 ...
 */
function countReachableCells(grid: CellType[], width: number, height: number, goals: Point[]): number {
  const visited = new Set<string>();
