 ...
 * @example
 */
function smoothPath(path: Point[], flowField: FlowCell[], width: number, height: number): Point[] {
  if (path.length <= 2) {
    return path;
  }
 ...
 * @example
 */
function calculateHeuristic(point: Point, flowField: FlowCell[], width: number, height: number): number {
