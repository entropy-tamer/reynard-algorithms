/**
 * @file Simplex Noise 3D Core Functions
 *
 ...
 */
function calculateGradientIndices(
  coords: { i: number; j: number; k: number; i1: number; j1: number; k1: number; i2: number; j2: number; k2: number },
  perm: number[],
  permMod12: number[]
): number[] {
  return [
    getGradientIndex(coords.i, coords.j, coords.k, 0, 0, 0, perm, permMod12),
    getGradientIndex(coords.i, coords.j, coords.k, coords.i1, coords.j1, coords.k1, perm, permMod12),
    getGradientIndex(coords.i, coords.j, coords.k, coords.i2, coords.j2, coords.k2, perm, permMod12),
    getGradientIndex(coords.i, coords.j, coords.k, 1, 1, 1, perm, permMod12),
  ];
}
 ...
 */
function calculateNoiseContributions(
  coords: {
    x0: number;
    y0: number;
    z0: number;
  ...
  const corners = [
    [coords.x0, coords.y0, coords.z0, 0],
    [coords.x1, coords.y1, coords.z1, 1],
    [coords.x2, coords.y2, coords.z2, 2],
    [coords.x3, coords.y3, coords.z3, 3],
  ];
  return corners.reduce(
    (sum, [x, y, z, idx]) => sum + getNoiseContribution(0.6 - x * x - y * y - z * z, grad3[gis[idx]], x, y, z),
    0
  );
}
