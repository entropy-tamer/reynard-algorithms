  ...
  validate(
    ...
    if (validationOptions.checkRectangle) {
      ...
      if (!MinimumBoundingBoxUtils.validateRectangle(result.rectangle, this.config)) {
        rectangleErrors.push(
          createValidationMessage("invalid-rectangle", ValidationSeverity.ERROR, "Invalid rectangle in result", {
            field: "rectangle",
            context: { rectangle: result.rectangle },
          })
        );
      }
    ...
    if (validationOptions.checkPerimeter) {
      ...
      if (result.perimeter < 0) {
        perimeterErrors.push(
          createValidationMessage("negative-perimeter", ValidationSeverity.ERROR, "Negative perimeter", {
            field: "perimeter",
            context: { perimeter: result.perimeter },
          })
        );
      }
    ...
    return createUnifiedValidationResult(errors.length === 0, errors, warnings, info, detailedResults, {
      duration,
