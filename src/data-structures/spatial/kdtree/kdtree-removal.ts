...
import type { Point, KdNode, KdTreeConfig, KdTreeStats, KdTreeResult, KdTreeEventHandler } from "./kdtree-types";
 ...
 * @example
 */
function findMin(node: KdNode, dimension: number, depth: number): KdNode {
 ...
 * @example
 */
export function clearTree(root: KdNode | null, stats: KdTreeStats, eventHandlers: KdTreeEventHandler[]): KdTreeResult {
  const startTime = performance.now();
