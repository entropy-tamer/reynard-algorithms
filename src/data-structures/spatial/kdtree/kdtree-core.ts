...
export class KdTree {
  ...
  insert(point: Point): KdTreeResult {
    const result = insertPoint(this.root, point, this.config, this.stats, this.eventHandlers, 0);
  ...
  insertBatch(points: Point[]): BatchOperationResult {
    const result = insertBatch(this.root, points, this.config, this.stats, this.eventHandlers);
  ...
  search(point: Point): boolean {
    const result = searchPoint(this.root, point, this.config, this.stats, this.eventHandlers);
  ...
  contains(point: Point): boolean {
    return containsPoint(this.root, point, this.config, this.stats, this.eventHandlers);
  }
  ...
  nearestNeighbor(queryPoint: Point, options: NearestNeighborOptions = {}): NearestNeighborResult {
    return findNearestNeighbor(this.root, queryPoint, this.config, this.stats, this.eventHandlers, options);
  }
  ...
  kNearestNeighbors(queryPoint: Point, options: Partial<KNearestNeighborsOptions> = {}): KNearestNeighborsResult {
    return findKNearestNeighbors(this.root, queryPoint, this.config, this.stats, this.eventHandlers, {
      k: options.k ?? 1,
      ...options,
    });
  }
  ...
  rangeQuery(bounds: BoundingBox, options: Partial<RangeQueryOptions> = {}): RangeQueryResult {
    return performRangeQuery(this.root, bounds, this.config, this.stats, this.eventHandlers, { bounds, ...options });
  }
  ...
  findPointsInRadius(center: Point, radius: number, options: Partial<RangeQueryOptions> = {}): RangeQueryResult {
    return findPointsInRadius(this.root, center, radius, this.config, this.stats, this.eventHandlers, options);
  }
  ...
  remove(point: Point): KdTreeResult {
    const result = removePoint(this.root, point, this.config, this.stats, this.eventHandlers);
