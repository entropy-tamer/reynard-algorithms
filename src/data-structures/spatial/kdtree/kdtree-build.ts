...
export function insertBatch(
  ...
  try {
    for (const point of points) {
      const insertResult = insertPoint(currentRoot, point, config, stats, eventHandlers, 0);
 ...
 * @example
 */
function buildTree(points: Point[], config: Required<KdTreeConfig>, depth: number): KdNode | null {
  if (points.length === 0) {
    return null;
  }
