...
import { traverseTree, getAllIntervals, findInterval, containsInterval } from "./interval-tree-traversal";
...
export class IntervalTree {
  ...
  insert(interval: Interval): boolean {
    const result = insertInterval(this.root, interval, this.config, this.stats, this.eventHandlers);
  ...
  delete(interval: Interval): boolean {
    const result = deleteInterval(this.root, interval, this.config, this.stats, this.eventHandlers);
  ...
  searchOverlapping(queryInterval: Interval): IntervalSearchResult {
    return searchOverlapping(this.root, queryInterval, this.config, this.stats, this.eventHandlers);
  }
  ...
  searchContaining(point: number): IntervalSearchResult {
    return searchContaining(this.root, point, this.config, this.stats, this.eventHandlers);
  }
  ...
  searchContainedIn(queryInterval: Interval): IntervalSearchResult {
    return searchContainedIn(this.root, queryInterval, this.config, this.stats, this.eventHandlers);
  }
  ...
  checkOverlap(interval: Interval): IntervalOverlapResult {
    return checkOverlap(this.root, interval, this.config, this.stats, this.eventHandlers);
  }
  ...
  insertBatch(intervals: Interval[]): BatchOperationResult {
    const result = insertBatch(this.root, intervals, this.config, this.stats, this.eventHandlers);
