...
import type { IntervalTreeNode, IntervalTreeEventHandler } from "./interval-tree-types";
 ...
 * @example
 */
export function balanceNode(node: IntervalTreeNode, eventHandlers: IntervalTreeEventHandler[] = []): IntervalTreeNode {
 ...
 * @example
 */
export function rightRotate(y: IntervalTreeNode, eventHandlers: IntervalTreeEventHandler[] = []): IntervalTreeNode {
  const x = y.left!;
  const T2 = x.right;
 ...
 * @example
 */
export function leftRotate(x: IntervalTreeNode, eventHandlers: IntervalTreeEventHandler[] = []): IntervalTreeNode {
  const y = x.right!;
  const T2 = y.left;
...
export function getBalanceStats(node: IntervalTreeNode | null): {
  ...
  const averageBalanceFactor =
    balanceFactors.length > 0 ? balanceFactors.reduce((sum, factor) => sum + factor, 0) / balanceFactors.length : 0;
 ...
 * @example
 */
function emitEvent(eventHandlers: IntervalTreeEventHandler[], type: IntervalTreeEventType, data: any): void {
