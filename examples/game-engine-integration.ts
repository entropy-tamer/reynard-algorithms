...
export class GameEngine {
  ...
  private detectCollisions(): Array<{a: GameObject, b: GameObject}> {
    ...
    const collisions = detectCollisions(objects.map(obj => ({
      id: obj.id,
      bounds: obj.bounds,
    })));
  ...
  private calculateSpatialDensity(): number {
    ...
    const objectArea = Array.from(this.state.objects.values())
      .reduce((sum, obj) => sum + (obj.bounds.width * obj.bounds.height), 0);
